...
def app():
    ...
    nia='[NIA](https://dot.gov.in/sites/default/files/NIA.pdf)'
    col1, mid, col2 = st.columns([2,1,20])
    with col1:
        st.image('dot.jpg', width=80)
    with col2:
         st.write("""## Spectrum Auction In Year 2016""")

    df=pd.read_csv('final_results_2016.csv')
    ...
    with stats:
        st.subheader(":rainbow[Statistics for 2016 Auction]")
        st.markdown("###### 👉 2G/3G/4G Auction")
        st.markdown("###### :key: Auction of multi bands- 700,800,900,1800,2100,2300 &2500 MHz")
        st.markdown("###### :bell: 41 percent spectrum was sold")
        st.markdown("###### :gem: Quantum of Spectrum put to auction: :blue[2354.75 MHz]")
        st.markdown("###### :battery: Quantum of Spectrum sold: :green[964.8 MHz] ")
        st.markdown("###### :nazar_amulet: Auction Format - Simultaneous Multi Round Auction(SMRA)")
        st.markdown("###### :chopsticks: Seven bidders participated in the auction")
        st.markdown("###### :jigsaw: All seven bidders won spectrum")        
        st.markdown("###### :wastebasket: Spectrum in 700 MHz and 900 MHz band were not sold.")
        st.markdown("###### :stopwatch: Auction lasted 6 days from 01st October to 6th October, 2016")
        st.markdown("###### :slightly_smiling_face: Auction ended in 31 clock rounds")
        #st.write( """#### """," | ",auction_timetable," | ")
        
        #Bring in the data
        time_table = pd.read_excel('time_table_auction_16.xlsx')
    ...
    with winners:
        ...
        #           text='Company_Name',color_continuous_scale=px.colors.sequential.Tealgrn,labels={'Company_Name':'Name of TSP'})
    ...
    with freq:
        ...
        dfg=pd.read_csv('final_results_2016_grouped_processed.csv')
        ...
        df_slice=pd.read_csv('Slices_2016_processed.csv')
        fig = px.treemap(df_slice, path=['Service_Area','Company_Name','Freq_slices'], title='Spectrum slices acquired by the TSPs in 2016 Auction',
                 )
        fig.update_layout(
        autosize=False,
        width=800,
